...
function M.setup()
	vim.g.openscad_cheatsheet_toggle_key = vim.g.openscad_cheatsheet_toggle_key or '<Enter>' -- <C-m>
...
function M.load()
	...
	M.set_mappings()
	if vim.g.openscad_open_on then
		M.window:open() -- open by default
	end
end
...
function M.exec_openscad()
	-- maybe just use fn.jobstart .. instead
